...
// --- Standalone UI Components (Replacing missing Shadcn files) ---
...
function LinePair({ original, pron, showPron, pronLabel }) {
  return (
    <div className="rounded-xl border border-slate-200 p-3 mb-2 bg-white shadow-sm">
...
function PrayerBlock({ title, body, pron, showPron, pronLabel }) {
  ...
  return (
    <div className="mb-6 text-left">
      <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">{title}</div>
      {lines.map((ln, idx) => (
...
export default function RosaryTrackerAppPrototype() {
  ...
  // Initial Load
  useEffect(() => {
    const raw = localStorage.getItem("rosary_final_build_v12");
    if (raw) {
      try {
  ...
  // Save Logic
  useEffect(() => {
    localStorage.setItem("rosary_final_build_v12", JSON.stringify({ dayKey, openingDone, closingDone, dailyIntention, startMystery, sets, prayerLang, showPron, pronType, history, lifetimeTotal }));
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 p-4 md:p-8 font-sans">
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-6 text-left">
          <div>
            ...
            <div className="grid gap-4 md:grid-cols-3 text-left">
              <Card className="md:col-span-1 h-fit">
                ...
                <CardContent className="space-y-5">
                  ...
                  <button disabled={!closingEnabled} onClick={() => setClosingDone(!closingDone)} className={`w-full p-3 rounded-xl border flex items-center justify-between text-sm font-bold transition-all ${closingDone ? "bg-emerald-50 border-emerald-200 text-emerald-700" : !closingEnabled ? "opacity-40 bg-slate-100" : "bg-white border-slate-100"}`}><span>Closing Prayers</span>{closingDone ? <CheckCircle2 className="text-emerald-500" /> : !closingEnabled ? <Lock className="h-4 w-4 text-slate-400" /> : <Circle className="text-slate-200" />}</button>
                </CardContent>
              </Card>
